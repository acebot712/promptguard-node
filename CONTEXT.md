# PromptGuard Node SDK

`promptguard-sdk` on npm. Secures an application's LLM calls, either by
standing in the request path or by scanning calls the application still makes
itself.

The Python SDK is a deliberate mirror of this one. Where a term below names
...
_Avoid_: shadow mode, shadow deployment — Shadow is a separate PromptGuard
product (Shadow AI devices, the Shadow fleet entitlement, the Shadow eval
corpus) and those names reach this repo through the generated API types. Also
avoid: observe mode, dry-run mode, passive mode.
...
### Staying in step with the Python SDK
...
spell the accessor differently — `getAppliedPatches()` here, `patched_sdks()`
in Python — but since contract v1.6.0 they are held to the same *vocabulary*,
so both report `bedrock` for the same provider.
