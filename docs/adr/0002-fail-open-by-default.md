...
Callers who want the opposite ask for it explicitly with `failOpen: false`,
which turns an unreachable Guard API into a thrown `GuardApiError`.
...
Because a thrown error is the fail-closed signal, anything that logs on this
path has to use the error's content-free description rather than its message,
which can carry the raw response body and echo prompt content.
