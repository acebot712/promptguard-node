...
/** What the calling account is entitled to, as the engine knew it here.

Composed from the same subscription row the scan was just counted against,
so it cannot be stale while a scan is in flight. Why it rides the scan
response rather than an endpoint of its own, and why it carries no price,
are ADR-0018; the reasoning lives there and not in four docstrings. */
export interface GuardEntitlement {
